...
此仓库是原版 `666ghj/MiroFish` 引擎的定制/克隆版本，包含以下关键升级与修复：
- **NVIDIA NIM API 深度兼容**: 专门修补了后端的 `llm_client.py`，拔除了硬编码的 JSON 对象格式约束（`response_format={"type": "json_object"}`）。这一修复一劳永逸地解决了在 GraphRAG 生成本体结构时，因调用 NVIDIA 平台非 OpenAI 原生大模型（如 `mistralai`、`nemotron` 系列）而导致的 `500 内部服务器报错`和断连崩溃问题。
- **预设模型优化**: 优化了开箱即用的默认参数，原生支持 NVIDIA 的最新超级模型，如 `nvidia/nemotron-3-super-120b-a12b` 及加速模拟节点 `nvidia/llama-3.1-nemotron-ultra-253b-v1`。
